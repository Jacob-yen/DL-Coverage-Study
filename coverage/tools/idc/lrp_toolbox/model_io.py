        ...
        with open(path,'r') as f:
            ...
            while len(line) > 0:
                if line.startswith(Linear.__name__): # @UndefinedVariable import error suppression for PyDev users
                    ...
                    '''
                    layer_name,m,n = line.split();   m = int(m); n = int(n)
                    layer_name = layer_name.split("@")[-1]
                    layer = Linear(layer_name,m,n)
                    ...
                    layer_name,h,w,d,n,s0,s1 = line.split()
                    layer_name = layer_name.split("@")[-1]
                    h = int(h); w = int(w); d = int(d); n = int(n); s0 = int(s0); s1 = int(s1)
                    layer = Convolution(name=layer_name,filtersize=(h,w,d,n), stride=(s0,s1))
                    ...
                    layer_name,h,w,s0,s1 = line.split()
                    layer_name = layer_name.split("@")[-1]
                    h = int(h); w = int(w); s0 = int(s0); s1 = int(s1)
                    layer = SumPool(name=layer_name,pool=(h,w),stride=(s0,s1))
                    modules.append(layer)
                    ...
                    layer_name = layer_name.split("@")[-1]
                    h = int(h); w = int(w); s0 = int(s0); s1 = int(s1)
                    layer = MaxPool(name=layer_name,pool=(h,w),stride=(s0,s1))
                    modules.append(layer)
                    c+=1 # one line of parameterized layer description

                elif line.startswith(Flatten.__name__): # @UndefinedVariable import error suppression for PyDev users
                    layer_name = line.split("@")[-1]
                    modules.append(Flatten(name=layer_name)) ; c+=1 #one line of parameterless layer description
                    print('flatten OK')
                elif line.startswith(Rect.__name__): # @UndefinedVariable import error suppression for PyDev users
                    layer_name = line.split("@")[-1]
                    modules.append(Rect(name=layer_name)) ; c+= 1 #one line of parameterless layer description
                # TODO: currently, tanh and softmax are not support by write_keras_txt therefore the lines below
                #  wouldn't be executed
                elif line.startswith(Tanh.__name__): # @UndefinedVariable import error suppression for PyDev users
                    layer_name = line.split("@")[-1]
                    modules.append(Tanh(layer_name)) ; c+= 1 #one line of parameterless layer description
                elif line.startswith(SoftMax.__name__): # @UndefinedVariable import error suppression for PyDev users
                    layer_name = line.split("@")[-1]
                    modules.append(SoftMax(layer_name)) ; c+= 1 #one line of parameterless layer description
                else:
    ...
    for layer, json_elem in zip(model.layers, json_obj['config']['layers']):
        if json_elem['class_name'] == 'Activation':
            if json_elem['config']['activation'] == 'softmax':
                ...
                txtfile.write('Rect'+f"@{json_elem['config']['name']}_self_add_relu")
        elif json_elem['class_name'] == 'Dense':
            txtfile.write(f"Linear@{json_elem['config']['name']} " + str(prev_layer.output_shape[-1]) + ' ' \
                + str(layer.output_shape[-1]))
        elif json_elem['class_name'] == 'Conv2D':
            txtfile.write(f"Convolution@{json_elem['config']['name']} " + str(json_elem['config']['kernel_size'][0])\
            ...
            txtfile.write(f"MaxPool@{json_elem['config']['name']} " + str(json_elem['config']['pool_size'][0]) + ' '\
                + str(json_elem['config']['pool_size'][1]) + ' ' \
                + str(json_elem['config']['strides'][0]) + ' ' \
                + str(json_elem['config']['strides'][1]))
        elif json_elem['class_name'] == 'Flatten':
            txtfile.write(json_elem['class_name']+f"@{json_elem['config']['name']}")
        else:
