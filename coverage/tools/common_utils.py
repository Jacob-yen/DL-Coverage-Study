...
def cached_sa(dataset_network, attack_type, test_size, classification=True):
    ...
    if classification:
        ...
        mdsa_file = os.path.join(root_dir, "files/cached_sa",
                                 f"{dataset_network}/mdsa_{dataset_network}_{attack_type}_{test_size}.npy")
        mdsa = np.load(mdsa_file, allow_pickle=True)
    else:
        dsa = mdsa = None
